      ...
      <div className="chat-messages">
        {Boolean(isFetchingConvMessages) && (
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%', minHeight: '300px' }}>
            <Spinner size={SpinnerSize.medium} aria-label="Fetching Chat messages" />
          </div>
        )}
        
        {/* NEW EMPTY STATE WITH GRAPH BLADES */}
        {!isFetchingConvMessages && !hasMessages && (
          <div className="initial-msg">
            <h2 style={{ margin: "0 0 8px 0", fontSize: "28px" }}>📊</h2>
            <Subtitle2 style={{ fontSize: "20px", color: "#2C2C2C", fontWeight: "600" }}>Start Chatting</Subtitle2>
            <Body1 style={{ textAlign: "center", color: "#6B7280", marginTop: "8px" }}>
              {chatLandingText || "You can ask questions around sales, products and orders."}
            </Body1>
            ...
            <div className="suggestion-chips-grid">
              {[
                { 
                  label: "Revenue", 
                  text: "Show total revenue by year for last 5 years",
                ...
                { 
                  label: "Products", 
                  text: "Top 10 products by revenue in the last year",
                ...
                { 
                  label: "Orders", 
                  text: "Show order status breakdown this quarter",
                ...
                { 
                  label: "Trends", 
                  text: "Compare Q3 vs Q4 sales performance",
                ...
                <button
                  ...
                  onClick={() => {
                    setUserMessage(q.text);
                    makeApiRequestWithCosmosDB(q.text, currentConversationId);
                  }}
                >
                  <div className="suggestion-icon">{q.icon}</div>
                  <div>
                    <div className="suggestion-chip-label">{q.label}</div>
                    <div className="suggestion-chip-text">{q.text}</div>
                  </div>
                </button>
              ))}
            </div>
        ...
        {!isFetchingConvMessages && messages.map((msg, index) => {
        ...
        {((generatingResponse && !isStreamingInProgress) || isChartLoading) && (
          <div className="assistant-message loading-indicator" style={{ border: "1px solid #E5E7EB", background: "#fff" }}>
            <div className="typing-indicator">
              <span className="generating-text" style={{ fontSize: "13px", color: "#6B7280" }}>
